...
export class ProductService {
  ...
  /**
   * Retrieves all the categories/genre of a specific type.
   *
   * @param productType - The type of products where the categories/genre have to be retrieved.
   * @returns A promise that resolves to the list of products or an error message.
   */
  async getCategoriesGenreOfType(
    productType: ProductType
  ): Promise<string[] | []> {
    try {
      // Sends the request to retrieve categories of a specific product type.
      const response = await axios.get(
        this.getCategoriesGenreByTypeURL + productType
      );
      return response.data;
    } catch (error: unknown) {
      let errorMessage = 'An unknown error occurred'; // Default error message.
      ...
      console.error(
        'Error while retrieving categories of a specific product type:',
        error
      );
